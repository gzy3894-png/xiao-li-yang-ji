...
- 基金搜索：代码 / 名称 / 拼音首字母模糊搜索，批量添加
- 自选列表：实时估值 + 涨跌幅 + 每分钟自动刷新（交易日）
- 指数行情：上证 / 深证 / 沪深300 / 创业板 / 科创50
- 收益计算：录入份额 / 成本价，自动计算当日估算收益与持有收益
- 估值源自选：天天基金官方估值 / 持仓加权估算（东财 / 腾讯 / 新浪行情），可自由切换或自动降级
- 基金详情：分时走势（交易时段本地记录）/ 官方估值 / 净值 / 涨幅走势
- 前十大持仓 + 各持仓股**实时涨跌**
- **截图导入持仓**（支付宝基金截图，本地 OCR 识别，不联网不传图）
- 数据本地存储，JSON 导入导出（预留）
- 国内直连，GitHub Actions 云端构建，Gitee App 内更新

## 技术栈

- Vue 3 + Vite + Pinia + Vue Router + ECharts
- Capacitor 6（Android 原生壳）
- tesseract.js 离线 OCR（chi_sim，识别支付宝截图）
- 自研原生插件 `capacitor-xiaoli-native`：绕过 WebView CORS 的 HTTP + APK 下载安装
